...
if [ -f "$MD_FILE" ]; then
  ...
  # repo commit hash (short) for the history repo
  ...
  # rotate backups: keep most recent $BACKUP_KEEP in repo, move older ones to local archive
  ...
  if [ ${#_bfiles[@]} -gt 0 ]; then
    # number to move = total - KEEP
    total=${#_bfiles[@]}
    move_count=$(( total - BACKUP_KEEP ))
    if [ "$move_count" -gt 0 ]; then
      for ((i=0;i<move_count;i++)); do
        src="${_bfiles[i]}"
        mv "$src" "$BACKUP_ARCHIVE_DIR/" || true
      done
    fi
  fi

  # ensure archive dir is ignored by git
  if ! grep -Fxq "${BACKUP_ARCHIVE_DIR#/}" "$BASE_DIR/.gitignore" 2>/dev/null; then
    printf "%s/\n" "${BACKUP_ARCHIVE_DIR#/}" >> "$BASE_DIR/.gitignore"
